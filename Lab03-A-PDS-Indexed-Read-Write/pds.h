...
//! (No loop here please, instead read all in one time).
// Close only the index file
int pds_open( char *repo_name, int rec_size );

//! One function for read index entries and one function for loading index entries into ndx_array
...
//! Use ftell to identify the current location.
// Add index entry to ndx_array using offset returned by ftell
// Increment record count
// Write the record at the end of the file
//! Return failure in case of duplicate key(No need to implement in lab 3a)
// index is corrupt means index is not in sync with the data file.
