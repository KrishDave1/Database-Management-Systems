...
void process_option1()
{
    ...
    printf("Enter parent key(1 to 10) and child key(1 to 10) for linking : ");
    scanf("%d%d", &parent_key, &child_key);
    if (parent_key > 0 && parent_key < 11){
        pds_link_rec(parent_key, child_key); // Calling the PDS functions to link the two records.
    }
    return;
